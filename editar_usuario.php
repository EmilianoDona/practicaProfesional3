       ...
       $status   = remove_junk($db->escape($_POST['status']));
         ...
         $result = $db->query($sql);
          if($result && $db->affected_rows() === 1){
            $session->msg('s',"USUARIO HA SIDO ACTUALIZADO");
     ...
     $h_pass   = sha1($password);
       ...
       $result = $db->query($sql);
        if($result && $db->affected_rows() === 1){
          ...
          $session->msg('d','NO SE PUDO ACTUALIZAR CONTRASEÑA, VUELVA A INTENTARLO, NO REPITA LOS CARACTERES!!');
          redirect('editar_usuario.php?id='.(int)$e_user['id'], false);
        }
  } else {
    $session->msg("d", $errors);
     ...
     <div class="panel panel-default">
       <div class="panel-heading">
        <strong>
          ...
          Actualiza cuenta de:&nbsp;<b style="font-weight: bold;color:blue;"><?php echo remove_junk(ucwords($e_user['name'])); ?></b>
        </strong>
       </div>
