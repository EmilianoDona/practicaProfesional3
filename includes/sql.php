...
// }
 ...
 function updateLastLogIn($user_id)
  ...
  function join_product3_table(){
   ...
   /*Material Especial*/
   function join_product2_table(){
     global $db;
     $sql  =" SELECT p.id,p.name,p.quantity,p.buy_price,p.titulo, p.titulo, p.descriptores, p.descriptores,p.sale_price,p.media_id,p.date,p.existencia,c.name";
    $sql  .=" AS categorie,m.file_name AS image";
    $sql  .=" FROM products2 p";
    $sql  .=" LEFT JOIN categories2 c ON c.id = p.categorie_id";
    $sql  .=" LEFT JOIN media m ON m.id = p.media_id";
    $sql  .=" ORDER BY p.id ASC";
    return find_by_sql($sql);

   }
// // Funcion Si Existencia
// function existencia_si(){
//   global $db;
//  $sql = "SELECT COUNT(id) FROM products2 WHERE existencia LIKE '%si%';";
//  return find_by_sql($sql);
// }
// // Funcion No Existencia
